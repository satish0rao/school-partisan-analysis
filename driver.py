...
tests = []
for (name,code) in [("hispanic",78),("hispanic_not",204),("hispanic_ok",224),
                    ("asian",76),("asian_not", 202),("asian_ok",222)]:
...
for (prefix,code,test_set) in tests:
    if not os.path.isfile("%s/kahuna.%s.%s.csv" % (root_for_outfiles,prefix,test_set)):
        if not os.path.isfile("school-data/%s.txt" % prefix):
            cmd = "cd school-data; python3 split_by_demo.py %s.txt %d; cd .." % (prefix, code)
            print("Running ", cmd)
            os.system(cmd)
        cmd = "python3 join_precinct_school_method2.py school-data/%s.txt %s" % (prefix,test_set)
        print("Running ", cmd)
        os.system(cmd)
...
for i in range(len(tests)):
    for j in range(len(tests)):
        ...
        cmd = "python3 quick_test.py \"%s/kahuna.%s.%s.csv\" \"%s/kahuna.%s.%s.csv\"" % (root_for_outfiles,prefix1,test_set1,root_for_outfiles,prefix2,test_set2)
        print("Running ", cmd)
        os.system(cmd)
